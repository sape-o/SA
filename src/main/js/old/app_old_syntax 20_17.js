...
class RepairInvoice extends React.Component {
    constructor(props) {
      ...
      this.state = {
        ...
        selectedEmp:'ทำไมยากจังเล้ยยยยยยยยยย',
        invoiceID:null,
        dateIn:null,
        dateOut:null,
    ...
    pushPage() {
      ...
      this.props.navigator.pushPage({ component: PrintRepairInvoice, 
           ...
           cusName:this.state.cusFirstName +'  '+ this.state.cusLastName,
    ...
    popPage() {
     ...
     handleClick2(cusFirstName,cusLastName,cusTel,Type,Brand,Model,Color,Problem,Note,cid,pid,sales_id,repair_id) {
      return function () {
        ...
        client({method: 'GET', path: '/customer/'+cid+'/product/'+pid+'/salesEmp/'+sales_id+'/repairEmp/'+repair_id}).done(
    ...
    handleClickTest() {
      ...
      //ons.notification.alert('มาแล้วๆๆ');
      this.setState.selectedEmp('อะไรวะ').done(
        ons.notification.alert('บันทึกสำเร็จ!')
      )
    }
      ...
      return(
        ...
        <Ons.ListItem key={row._links.self.href} tappable>
            ...
            <label className='left'>
              <Ons.Radio
                inputId={`radio-${row}`}
                //checked={row === this.state.employee}
                onChange={this.handleClick.bind(this, row)}
              />
            </label>
...
*/} 
            ...
            <Ons.Button onClick={this.handleClick2(this.state.cusFirstName,this.state.cusLastName,this.state.cusTel,
                  this.state.Type,this.state.Brand,this.state.Model,this.state.Color,this.state.Problem,this.state.Note,
                  this.state.cid,this.state.pid,this.state.salesEmp_id,this.state.repairEmp_id)}>
                บันทึกข้อมูล
            </Ons.Button>


            <Ons.Button onClick={evt => this.setState({selectedEmp: 'Hey'})}>
                บันทึกข้อมูลX
            </Ons.Button>
